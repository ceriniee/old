...
class Setting(object):
    def __init__(self, sp_rate=0.9, lr=0.01, prot_drug_dim=16, n_embed=48, n_hid1=32, n_hid2=16, num_base=32) -> None: #return값 명시
        super().__init__()
...
class TIP(nn.Module): #Pytorch 신경망 클래스, 상속하여 네트워크 구현
    def __init__(self, settings:Setting, device, mod='cat', data_path='./data/data_dict.pkl', ) -> None:
        super().__init__()
        self.mod = mod
        assert mod in {'cat', 'add'} #cat이나 add 아니면 에러 발생
    ...
    def __prepare_data(self, data_path, sp_rate):
        ...
        self.test_neg_index = typed_negative_sampling(data.dd_test_idx, data.n_drug, data.dd_test_range) #pos_edge_index, num_nodes, range_list return -> torch.cat은 concat 약자,  dim=1 즉 1차원 tensor로
        #print(self.test_neg_index.dim(), self.test_neg_index.shape, self.test_neg_index.dtype)
        #print(type(data))
        return data
    ...
    def __prepare_model(self):
        ...
        #print(self.data.d_feat.dtype, self.data.dd_train_idx.dtype, self.data.pp_train_indices.dtype, self.data.dp_edge_index.dtype)
...
class FMEncoder(torch.nn.Module):
    ...
    def forward(self, x_drug, dd_edge_index, dd_edge_type, dd_range_list, d_norm,
        ...
        #print(dp_edge_index, dp_edge_index.shape)
        x_prot = self.hgcn(x_prot, dp_edge_index, dp_range_list) #edge_index가 int32면 안돌아감!! ㅠ
        # x_prot = checkpoint(self.hgcn, torch.cat((x_prot, self.hdrug)), dp_edge_index, dp_range_list)
